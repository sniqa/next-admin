      ...
      if (errResult.length === 0) {
        ...
        toast(
          <div className="flex flex-col gap-1">
            ...
            <Anchor
              className="text-sm"
              onClick={() => onUploadFaildResult(errResult as any)}
            >
...
const getDataFromDeviceImportTableRows = (rows: Row[]) => {
  return rows.map((row) => ({
    ...
    username: turnRowEmptyValue(row[3]),
    department: turnRowEmptyValue(row[4]),
    network: turnRowEmptyValue(row[5]),
    ipAddress: turnRowEmptyValue(row[6]),
    deviceStatus: turnRowEmptyValue(row[7]),
    mac: turnRowEmptyValue(row[8]),
    diskSerialNumber: turnRowEmptyValue(row[9]),
    category: turnRowEmptyValue(row[10]),
    model: turnRowEmptyValue(row[11]),
    remark: turnRowEmptyValue(row[12]),
  }));
};
